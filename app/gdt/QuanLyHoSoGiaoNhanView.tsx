...
import { Button, Input, Table, Tabs, Tag, Space, Card, Row, Col, Statistic, Dropdown, ConfigProvider, Typography, Select, Segmented, Radio, Badge as AntBadge, Breadcrumb, Tooltip } from "antd";
...
function ModalTaoCongVan({ record, onClose, onConfirm }: { record?: any; onClose: () => void; onConfirm: (config: any) => void }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 1100, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: F }}>
      <div style={{ background: "#ffffffff", borderRadius: 10, width: 580, maxWidth: "92vw", padding: 12, boxShadow: "0 20px 40px rgba(0,0,0,0.25)", fontFamily: F, display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <div
                ...
                <div style={{ display: "flex", justifyContent: "space-between", marginTop: 40 }}>
                  <div style={{ fontSize: 14, fontStyle: "italic", lineHeight: 1.6 }}>
                    ...
                    <div>- Vụ 9 - Viện kiểm sát nhân dân tối cao (kèm hồ sơ vụ án);</div>
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 1800, background: "rgba(0,0,0,0.45)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: F }}>
      <div style={{ background: "#fff", borderRadius: 8, width: 460, boxShadow: "0 10px 40px rgba(0,0,0,0.2)", display: "flex", flexDirection: "column" }}>
        <div style={{ padding: "16px 20px", borderBottom: `1px solid ${BORDER}`, background: "#fef2f2", borderRadius: "8px 8px 0 0" }}>
          <div style={{ fontWeight: 700, fontSize: 16, color: "#991b1b" }}>Trả lại hồ sơ</div>
...
function ModalNhapTayHoSoDen({ loaiHoSo, onClose, onSave }: { loaiHoSo: "tu-hinh" | "khang-nghi"; onClose: () => void; onSave: (data: any) => void }) {
  const [form, setForm] = React.useState({ loaiDonVi: "Viện kiểm sát", tenDonVi: "", soCongVan: "", ngayGui: "", nguoiGui: "" });
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 1800, background: "rgba(0,0,0,0.45)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: F }}>
      <div style={{ background: "#fff", borderRadius: 8, width: 520, maxHeight: "90vh", overflow: "auto", boxShadow: "0 10px 40px rgba(0,0,0,0.2)", display: "flex", flexDirection: "column" }}>
        <div style={{ padding: "16px 20px", borderBottom: `1px solid ${BORDER}`, background: "#f8fafc", borderRadius: "8px 8px 0 0" }}>
          ...
          <div style={{ fontSize: 14, color: MUTED, marginTop: 4 }}>Dùng cho hồ sơ nhận từ Viện kiểm sát hoặc cơ quan ngoài hệ thống QLA.</div>
        </div>
...
export function QuanLyHoSoGiaoNhanView({ userRole, onTaoCongVan, onThemHoSo }: { userRole?: UserRoleType; onTaoCongVan?: (config?: any) => void; onThemHoSo?: () => void }) {
  type MainTab = "tu-hinh" | "khang-nghi" | "xet-xu-lai";
  const [activeMain, setActiveMain] = React.useState<MainTab>("tu-hinh");
  const [tab, setTab] = React.useState("khang-nghi");
  const [tuHinhSubTab, setTuHinhSubTab] = React.useState<"den" | "di">("den");
  const [congVanSubTab, setCongVanSubTab] = React.useState<"den" | "di">("den");
  const [filterStatus, setFilterStatus] = React.useState("tat-ca");
  ...
  const [listTuHinhDen, setListTuHinhDen] = React.useState([
    ...
    { id: 902, maDon: "TH-DEN-002", soCongVan: "CV-2026/THA-112", ngayGui: "05/08/2026", donViGui: "Viện kiểm sát tỉnh Bắc Ninh", loaiDonVi: "Viện kiểm sát", nguoiGui: "Trần Thị Hoa", soBA: "45/2026/HS-ST", toaRaBanAn: "TAND tỉnh Bắc Ninh", tenBiAn: "Phạm Văn Cường", trangThai: "Đã tiếp nhận", ngayNhan: "07/08/2026", nguoiNhan: "Lý Thái Phúc" },
    { id: 903, maDon: "TH-DEN-003", soCongVan: "CV-2026/THA-088", ngayGui: "20/07/2026", donViGui: "TAND tỉnh Long An", loaiDonVi: "Tòa án cấp dưới", nguoiGui: "Hoàng Minh Tuấn", soBA: "11/2026/HS-ST", toaRaBanAn: "TAND tỉnh Long An", tenBiAn: "Nguyễn Thanh Liêm", trangThai: "Trả lại", ngayNhan: "--", nguoiNhan: "--" },
    { id: 904, maDon: "TH-DEN-004", soCongVan: "CV-2026/THA-134", ngayGui: "15/09/2026", donViGui: "Viện kiểm sát nhân dân tối cao", loaiDonVi: "Viện kiểm sát", nguoiGui: "Lê Thị Bình", soBA: "78/2026/HS-PT", toaRaBanAn: "TAND Cấp cao tại TP.HCM", tenBiAn: "Võ Văn Đại", trangThai: "Chưa tiếp nhận", ngayNhan: "--", nguoiNhan: "--" },
  ]);

  const [listKNDi, setListKNDi] = React.useState([
    { id: 1, soHieuKN: "10/2026/QĐKN", ngayKN: "25/07/2026", nguoiKN: "Chánh án TAND Tối cao", soBA: "124/2026/HS-ST", toaRaBanAn: "TAND tỉnh Hà Nam", donViNhan: "Viện kiểm sát nhân dân tối cao", soCVChuyen: "CV-2026/088", ngayChuyen: "22/07/2026", trangThai: "Đã chuyển" },
    { id: 2, soHieuKN: "12/2026/QĐKN", ngayKN: "18/06/2026", nguoiKN: "Viện trưởng Viện kiểm sát nhân dân tối cao", soBA: "102/2026/DS-ST", toaRaBanAn: "TAND quận Hoàn Kiếm", donViNhan: "TAND TP Hà Nội", soCVChuyen: "", ngayChuyen: "--", trangThai: "Chưa chuyển" },
    ...
    { id: 4, soHieuKN: "19/2026/QĐKN", ngayKN: "05/08/2026", nguoiKN: "Viện trưởng Viện kiểm sát nhân dân tối cao", soBA: "78/2026/HS-PT", toaRaBanAn: "TAND tỉnh Bắc Ninh", donViNhan: "TAND Cấp cao tại Hà Nội", soCVChuyen: "", ngayChuyen: "--", trangThai: "Chưa chuyển" },
  ]);

  const [listKNDen, setListKNDen] = React.useState([
    { id: 101, soHieuKN: "08/2026/QĐKN", ngayKN: "03/07/2026", nguoiKN: "Viện trưởng Viện kiểm sát tỉnh Bắc Ninh", soBA: "236/2026/HS-PT", toaRaBanAn: "TAND tỉnh Bắc Ninh", donViGui: "Viện kiểm sát tỉnh Bắc Ninh", ngayNhan: "24/07/2026", nguoiNhan: "Lý Thái Phúc", trangThai: "Đã tiếp nhận" },
    { id: 102, soHieuKN: "14/2026/QĐKN", ngayKN: "25/04/2026", nguoiKN: "Chánh án TAND tỉnh Bắc Ninh", soBA: "18/2026/KDTM-ST", toaRaBanAn: "TAND tỉnh Bắc Ninh", donViGui: "TAND tỉnh Bắc Ninh", ngayNhan: "--", nguoiNhan: "--", trangThai: "Chưa tiếp nhận" },
    { id: 103, soHieuKN: "21/2026/QĐKN", ngayKN: "12/03/2026", nguoiKN: "Chánh án TAND quận Đống Đa", soBA: "88/2026/HNGĐ-PT", toaRaBanAn: "TAND quận Đống Đa", donViGui: "Văn phòng LS Trí Đức", ngayNhan: "--", nguoiNhan: "--", trangThai: "Chưa tiếp nhận" },
    { id: 104, soHieuKN: "27/2026/QĐKN", ngayKN: "01/08/2026", nguoiKN: "Viện trưởng Viện kiểm sát nhân dân tối cao", soBA: "174/2026/HS-ST", toaRaBanAn: "TAND tỉnh Hưng Yên", donViGui: "Viện kiểm sát nhân dân tối cao", ngayNhan: "--", nguoiNhan: "--", trangThai: "Chưa tiếp nhận" },
    { id: 109, soHieuKN: "41/2026/QĐKN-HC", ngayKN: "06/08/2026", nguoiKN: "Chánh án TAND tỉnh Lâm Đồng", soBA: "29/2026/HC-PT", toaRaBanAn: "TAND tỉnh Lâm Đồng", donViGui: "TAND tỉnh Lâm Đồng", ngayNhan: "07/08/2026", nguoiNhan: "Nguyễn Tiến Mạnh", trangThai: "Đã tiếp nhận" },
  ]);

  const [listXetXuLai, setListXetXuLai] = React.useState([
    // Vụ hình sự - chuyển theo bị cáo: mỗi bị cáo 1 dòng, cấp xử lại & đơn vị nhận riêng
    { id: 201, maVuAn: "VA26-000035", tenVuAn: "Chu Văn An và đồng phạm giết người", loaiAn: "Hình sự", biCao: "Chu Văn An", ketQuaGDT: "Hủy bản án PT đối với bị cáo Chu Văn An, xét xử lại ST", capXuLai: "Sơ thẩm", donViNhan: "TAND tỉnh Long An", soQDGDT: "01/2026/QĐ-GĐT", ngayBanHanh: "15/08/2026", toaRaQDGDT: "TAND Cấp cao tại Hà Nội", trangThai: "Chưa chuyển" },
    { id: 204, maVuAn: "VA26-000035", tenVuAn: "Chu Văn An và đồng phạm giết người", loaiAn: "Hình sự", biCao: "Trần Đức Minh", ketQuaGDT: "Hủy bản án PT đối với bị cáo Trần Đức Minh, xét xử lại PT", capXuLai: "Phúc thẩm", donViNhan: "TAND Cấp cao tại TP.HCM", soQDGDT: "01/2026/QĐ-GĐT", ngayBanHanh: "15/08/2026", toaRaQDGDT: "TAND Cấp cao tại Đà Nẵng", trangThai: "Chưa chuyển" },

    // Vụ hình sự - GĐ kiểm tra về hình sự
    { id: 202, maVuAn: "VA26-000042", tenVuAn: "Nguyễn Văn B và đồng phạm cướp tài sản", loaiAn: "Hình sự", biCao: "Nguyễn Văn B", ketQuaGDT: "Hủy bản án ST, xét xử lại PT", capXuLai: "Phúc thẩm", donViNhan: "TAND Cấp cao tại Hà Nội", soQDGDT: "02/2026/QĐ-GĐT", ngayBanHanh: "20/08/2026", toaRaQDGDT: "TAND Tối cao", trangThai: "Đã chuyển" },
    { id: 206, maVuAn: "VA26-000042", tenVuAn: "Nguyễn Văn B và đồng phạm cướp tài sản", loaiAn: "Hình sự", biCao: "Phạm Thế Anh", ketQuaGDT: "Hủy bản án ST, xét xử lại ST", capXuLai: "Sơ thẩm", donViNhan: "TAND TP Hà Nội", soQDGDT: "02/2026/QĐ-GĐT", ngayBanHanh: "20/08/2026", toaRaQDGDT: "TAND Cấp cao tại Hà Nội", trangThai: "Chưa chuyển" },
    // Vụ dân sự - giữ logic cũ, 1 dòng/vụ, không có bị cáo
    { id: 203, maVuAn: "VA26-000051", tenVuAn: "Tranh chấp hợp đồng mua bán", loaiAn: "Dân sự", biCao: "", ketQuaGDT: "Hủy quyết định GĐT, xét xử lại ST", capXuLai: "Sơ thẩm", donViNhan: "TAND TP Hà Nội", soQDGDT: "03/2026/QĐ-GĐT", ngayBanHanh: "01/09/2026", toaRaQDGDT: "TAND Cấp cao tại Hà Nội", trangThai: "Đã tiếp nhận (PT/ST)" },
  ]);

  const [listCongVanDen, setListCongVanDen] = React.useState([
    { id: 301, soCV: "456/CV-VKS", ngayCV: "10/09/2026", nguoiGui: "Viện trưởng Viện kiểm sát nhân dân tối cao", soBA: "12/2026/HS-PT", ngayBA: "15/02/2026", toaRaBanAn: "TAND Cấp cao tại Hà Nội", donViGui: "Viện kiểm sát nhân dân tối cao", ngayNhan: "--", nguoiNhan: "--", trangThai: "Chưa tiếp nhận" },
    { id: 302, soCV: "889/CV-TAND", ngayCV: "05/09/2026", nguoiGui: "Chánh án TAND tỉnh Bắc Ninh", soBA: "45/2026/HS-ST", ngayBA: "20/03/2026", toaRaBanAn: "TAND tỉnh Bắc Ninh", donViGui: "TAND tỉnh Bắc Ninh", ngayNhan: "07/09/2026", nguoiNhan: "Trần Văn A", trangThai: "Đã tiếp nhận" },
  ]);

  const [listCongVanDi, setListCongVanDi] = React.useState([
    { id: 401, soCV: "112/CV-TANDTC", ngayCV: "12/09/2026", nguoiGui: "Chánh án TAND Tối cao", soBA: "124/2026/HS-ST", ngayBA: "05/05/2026", toaRaBanAn: "TAND tỉnh Hà Nam", donViNhan: "Viện kiểm sát nhân dân tối cao", ngayChuyen: "12/09/2026", trangThai: "Đã chuyển" },
    { id: 402, soCV: "115/CV-TANDTC", ngayCV: "14/09/2026", nguoiGui: "Chánh án TAND Tối cao", soBA: "102/2026/DS-ST", ngayBA: "12/04/2026", toaRaBanAn: "TAND quận Hoàn Kiếm", donViNhan: "TAND TP Hà Nội", ngayChuyen: "--", trangThai: "Chưa chuyển" },
  ]);

  const thuHinhChua = listTuHinhDen.filter(x => x.trangThai === "Chưa tiếp nhận").length;
  const knDenCho = listKNDen.filter(x => x.trangThai === "Chưa tiếp nhận").length;
  const knDiCho = listKNDi.filter(x => x.trangThai === "Chưa chuyển" || x.trangThai === "Chưa chuyển").length;
  const xxlChoXuLy = listXetXuLai.filter(x => x.trangThai === "Chưa chuyển").length;
  const cvDenChua = listCongVanDen.filter(x => x.trangThai === "Chưa tiếp nhận" || x.trangThai === "Chưa tiếp nhận").length;
  const cvDiCho = listCongVanDi.filter(x => x.trangThai === "Chưa chuyển" || x.trangThai === "Chưa chuyển").length;

  const getFilteredData = (data: any[]) => {
    let result = data;
    if (filterStatus !== "tat-ca") {
      if (filterStatus === "chua-xu-ly") result = result.filter(r => r.trangThai.includes("Chưa") || r.trangThai.includes("Chờ"));
      else if (filterStatus === "da-xu-ly") result = result.filter(r => r.trangThai.includes("Đã chuyển") || r.trangThai.includes("Đã tiếp nhận"));
      else if (filterStatus === "da-tra") result = result.filter(r => r.trangThai.includes("Từ chối") || r.trangThai.includes("trả"));
      else if (filterStatus === "da-nhan") result = result.filter(r => r.trangThai.includes("Đã tiếp nhận"));
    }
    if (searchText) {
  ...
  const allRows = [
    ...
    ...listCongVanDen.map(r => ({ ...r, loaiHoSo: "Công văn trao đổi", chieu: "Đến", targetTab: "cong-van" as MainTab })),
    ...listCongVanDi.map(r => ({ ...r, loaiHoSo: "Công văn trao đổi", chieu: "Đi", targetTab: "cong-van" as MainTab })),
  ];
  ...
  const handleTuChoiTuHinh = (id: number, lyDo: string) => { setListTuHinhDen(prev => prev.map(r => r.id === id ? { ...r, trangThai: "Trả lại" } : r)); setShowTuChoi(false); alert("Đã Trả lại.\nLý do: " + lyDo); };
  const handleNhanKNDen = (id: number) => setListKNDen(prev => prev.map(r => r.id === id ? { ...r, trangThai: "Đã tiếp nhận", ngayNhan: new Date().toLocaleDateString("vi-VN"), nguoiNhan: "Lý Thái Phúc" } : r));
  ...
  const tongHopCols = [
    ...
    { title: "Chi tiết", key: "action", align: "center" as const, width: 60, render: (_: any, r: any) => <Tooltip title="Xem chi tiết"><Button type="text" shape="circle" icon={<Eye size={16} color="#0f766e" />} onClick={() => { setActiveMain(r.targetTab); if (r.targetTab === "khang-nghi") setKNSubTab(r.chieu === "Đi" ? "di" : "den"); }} /></Tooltip> }
  ];

  const tuHinhCols = [
    { title: "STT", key: "stt", width: 60, align: "center" as const, render: (_: any, __: any, i: number) => i + 1 },
    { title: "Thông tin hồ sơ", key: "thongTinHS", render: (r: any) => <div style={{ lineHeight: 1.6 }}><div><span style={{ color: MUTED }}>Ngày gửi: </span><span>{r.ngayGui}</span></div><div><span style={{ color: MUTED }}>Đơn vị gửi: </span><span>{r.donViGui}</span></div></div> },
    { title: "Thông tin BA/QĐ", key: "thongTinBA", render: (r: any) => <div style={{ lineHeight: 1.6 }}><div><span style={{ color: MUTED }}>Số BA/QĐ: </span><strong>{r.soBA}</strong></div><div><span style={{ color: MUTED }}>Ngày BA/QĐ: </span><span>{r.ngayBA || "12/01/2026"}</span></div><div><span style={{ color: MUTED }}>Tại: </span><span>{r.toaRaBanAn}</span></div></div> },
    { title: "Bị án & Tội danh", key: "biAnToiDanh", render: (r: any) => <div style={{ lineHeight: 1.6 }}><div><span style={{ fontWeight: 600, color: TEXT }}>{r.tenBiAn}</span><span style={{ color: MUTED }}> – </span><span style={{ color: RED, fontWeight: 500 }}>{r.toiDanh || "Giết người"}</span></div></div> },
    { title: "Trạng thái", dataIndex: "trangThai", key: "trangThai", align: "center" as const, width: 160, render: (v: string) => <TrangThaiBadge tt={v} /> },
    {
      title: "Thao tác", key: "action", align: "center" as const, width: 100, render: (_: any, r: any) => (
        r.trangThai === "Chưa tiếp nhận" ? (
          <Space size="small">
            <Tooltip title="Tiếp nhận"><Button type="text" shape="circle" icon={<CheckCheck size={16} color="#16a34a" />} onClick={() => handleTiepNhanTuHinh(r.id)} /></Tooltip>
            <Tooltip title="Từ chối"><Button type="text" shape="circle" icon={<Undo2 size={16} color="#dc2626" />} onClick={() => { setSelectedRecord(r); setShowTuChoi(true); }} /></Tooltip>
          </Space>
  ...
  const knDenCols = [
    ...
    { title: "Số bản án / Quyết định", key: "soBA", render: (r: any) => <div><div style={{ color: "#2563eb", fontWeight: 600 }}>Số: {r.soBA}</div><div style={{ fontSize: 13, color: "#6b7280" }}>Ngày: {r.ngayBA || "15/02/2026"}</div><div style={{ fontSize: 13, color: "#6b7280" }}>Tòa ra BA/QĐ: {r.toaRaBanAn}</div></div> },
    ...
    {
      title: "Thao tác", key: "action", align: "center" as const, width: 100, render: (_: any, r: any) => r.trangThai === "Chưa tiếp nhận" && (
        <Space size="small">
          <Tooltip title="Nhận hồ sơ"><Button type="text" shape="circle" icon={<CheckCheck size={16} color="#16a34a" />} onClick={() => handleNhanKNDen(r.id)} /></Tooltip>
          <Tooltip title="Trả hồ sơ"><Button type="text" shape="circle" icon={<Undo2 size={16} color="#dc2626" />} onClick={() => { setSelectedRecord(r); setShowTraHoSo(true); }} /></Tooltip>
        </Space>
      )
    }
  ];
  ...
  const knDiCols = [
    { title: "STT", key: "stt", width: 60, align: "center" as const, render: (_: any, __: any, i: number) => i + 1 },
    { title: "Thông tin kháng nghị", key: "thongTin", render: (r: any) => <div><div style={{ color: RED, fontWeight: 600 }}>{r.soHieuKN}</div><div style={{ fontSize: 13, color: "#6b7280" }}>Ngày: {r.ngayKN}</div><div style={{ fontSize: 13 }}>{r.nguoiKN}</div></div> },
    { title: "Số bản án / Quyết định", key: "soBA", render: (r: any) => <div><div style={{ color: "#2563eb", fontWeight: 600 }}>Số: {r.soBA}</div><div style={{ fontSize: 13, color: "#6b7280" }}>Ngày: {r.ngayBA || "20/03/2026"}</div><div style={{ fontSize: 13, color: "#6b7280" }}>Tòa ra BA/QĐ: {r.toaRaBanAn}</div></div> },
    ...
    { title: "Trạng thái", dataIndex: "trangThai", key: "trangThai", align: "center" as const, width: 140, render: (v: string) => <TrangThaiBadge tt={v} /> },
    { title: "Thao tác", key: "action", align: "center" as const, width: 80, render: (_: any, r: any) => <Tooltip title="Trình ký"><Button type="text" shape="circle" icon={<PenLine size={16} color="#d97706" />} onClick={() => { setSelectedRecord(r); setShowTrinhKy(true); }} /></Tooltip> }
  ];

  const isHinhSu = (r: any) => {
    const la = (r.loaiAn || "").toLowerCase();
    return la.includes("hình sự");
  };

  const xxlCols = [
    { title: "STT", key: "stt", width: 60, align: "center" as const, render: (_: any, __: any, i: number) => i + 1 },
    { title: "Vụ án", key: "vuAn", render: (r: any) => <div><div style={{ fontWeight: 600 }}>{r.maVuAn}</div><div style={{ fontSize: 13, color: "#6b7280" }}>{r.tenVuAn}</div>{r.loaiAn && <Tag color={isHinhSu(r) ? "volcano" : "cyan"} style={{ marginTop: 4 }}>{r.loaiAn}</Tag>}</div> },
    { title: "Bị cáo / Đương sự", key: "biCao", width: 160, render: (r: any) => isHinhSu(r) && r.biCao ? <Text strong style={{ color: "#b91c1c" }}>{r.biCao}</Text> : <Text type="secondary">—</Text> },
    { title: "Kết quả GĐT / Quyết định", key: "kq", render: (r: any) => <div><div style={{ color: RED, fontWeight: 600 }}>{r.ketQuaGDT}</div><div style={{ fontSize: 13, color: "#6b7280" }}>Số: {r.soQDGDT}</div><div style={{ fontSize: 13, color: "#6b7280" }}>Ngày: {r.ngayBanHanh}</div><div style={{ fontSize: 13, color: "#6b7280" }}>Tòa ra BA/QĐ: {r.toaRaQDGDT || r.toaRaBanAn}</div></div> },
    { title: "Cấp xử lại", dataIndex: "capXuLai", key: "capXuLai", align: "center" as const, width: 130, render: (v: string) => <Tag color={v === "Sơ thẩm" ? "gold" : v === "Phúc thẩm" ? "blue" : "default"} style={{ fontWeight: 600 }}>{v}</Tag> },
    { title: "Đơn vị nhận", dataIndex: "donViNhan", key: "donViNhan" },
    { title: "Trạng thái", dataIndex: "trangThai", key: "trangThai", align: "center" as const, width: 160, render: (v: string) => <TrangThaiBadge tt={v} /> },
    { title: "Thao tác", key: "action", align: "center" as const, width: 80, render: (_: any, r: any) => r.trangThai === "Chưa chuyển" ? <Tooltip title="Xác nhận chuyển"><Button type="text" shape="circle" icon={<Send size={16} color="#16a34a" />} onClick={() => handleXacNhanChuyen(r.id)} /></Tooltip> : <Tooltip title="Xem vụ án"><Button type="text" shape="circle" icon={<Eye size={16} color="#0f766e" />} onClick={() => window.location.href = `#/vu-an/${r.id}`} /></Tooltip> }
  ];

  const cvDenCols = [
    { title: "STT", key: "stt", width: 60, align: "center" as const, render: (_: any, __: any, i: number) => i + 1 },
    { title: "Thông tin công văn", key: "thongTin", render: (r: any) => <div><div style={{ color: RED, fontWeight: 600 }}>{r.soCV}</div><div style={{ fontSize: 13, color: "#6b7280" }}>Ngày: {r.ngayCV}</div><div style={{ fontSize: 13 }}>{r.nguoiGui}</div></div> },
    { title: "Số bản án / Quyết định", key: "soBA", render: (r: any) => <div><div style={{ color: "#2563eb", fontWeight: 600 }}>Số: {r.soBA}</div><div style={{ fontSize: 13, color: "#6b7280" }}>Ngày: {r.ngayBA}</div><div style={{ fontSize: 13, color: "#6b7280" }}>Tòa ra BA/QĐ: {r.toaRaBanAn}</div></div> },
    { title: "Đơn vị gửi / Người nhận", key: "donVi", render: (r: any) => <div><div><b>Gửi:</b> {r.donViGui}</div><div style={{ fontSize: 13, color: "#6b7280" }}><b>Nhận:</b> {r.nguoiNhan} ({r.ngayNhan})</div></div> },
    { title: "Trạng thái", dataIndex: "trangThai", key: "trangThai", align: "center" as const, width: 140, render: (v: string) => <TrangThaiBadge tt={v} /> },
    {
      title: "Thao tác", key: "action", align: "center" as const, width: 100, render: (_: any, r: any) => r.trangThai === "Chưa tiếp nhận" && (
        <Space size="small">
          <Tooltip title="Nhận công văn"><Button type="text" shape="circle" icon={<CheckCheck size={16} color="#16a34a" />} /></Tooltip>
          <Tooltip title="Trả công văn"><Button type="text" shape="circle" icon={<Undo2 size={16} color="#dc2626" />} onClick={() => { setSelectedRecord(r); setShowTraHoSo(true); }} /></Tooltip>
        </Space>
      )
    }
  ];
  ...
  const cvDiCols = [
    ...
    { title: "Thông tin công văn", key: "thongTin", render: (r: any) => <div><div style={{ color: RED, fontWeight: 600 }}>{r.soCV}</div><div style={{ fontSize: 13, color: "#6b7280" }}>Ngày: {r.ngayCV}</div><div style={{ fontSize: 13 }}>{r.nguoiGui}</div></div> },
    { title: "Số bản án / Quyết định", key: "soBA", render: (r: any) => <div><div style={{ color: "#2563eb", fontWeight: 600 }}>Số: {r.soBA}</div><div style={{ fontSize: 13, color: "#6b7280" }}>Ngày: {r.ngayBA}</div><div style={{ fontSize: 13, color: "#6b7280" }}>Tòa ra BA/QĐ: {r.toaRaBanAn}</div></div> },
    { title: "Đơn vị nhận", key: "donVi", render: (r: any) => <div><div><b>Nhận:</b> {r.donViNhan}</div><div style={{ fontSize: 13, color: "#6b7280" }}><b>Ngày chuyển:</b> {r.ngayChuyen}</div></div> },
    { title: "Trạng thái", dataIndex: "trangThai", key: "trangThai", align: "center" as const, width: 140, render: (v: string) => <TrangThaiBadge tt={v} /> },
    { title: "Thao tác", key: "action", align: "center" as const, width: 80, render: (_: any, r: any) => <Tooltip title="Trình ký"><Button type="text" shape="circle" icon={<PenLine size={16} color="#d97706" />} onClick={() => { setSelectedRecord(r); setShowTrinhKy(true); }} /></Tooltip> }
  ];
  ...
  return (
    <ConfigProvider theme={{ token: { colorPrimary: RED, fontFamily: F, borderRadius: 6 } }}>
      <div style={{ padding: 24 }}>
        <Breadcrumb items={[{ title: 'Quản lý án GĐT/TT' }, { title: 'Quản lý hồ sơ giao nhận' }]} style={{ marginBottom: 16 }} />
        <Typography.Title level={4} style={{ marginBottom: 5 }}>Quản lý hồ sơ giao nhận</Typography.Title>
        <div style={{ display: 'flex', gap: 16, marginBottom: 24, flexWrap: 'wrap' }}>
          {[
            { key: "khang-nghi", title: "Hồ sơ kháng nghị", desc: "Quản lý hồ sơ kháng nghị", icon: <FolderCheck size={24} /> },
            { key: "xet-xu-lai", title: "Hồ sơ GĐT/TT xét xử lại", desc: "Hồ sơ xét xử lại", icon: <RotateCcw size={24} /> },
            { key: "cong-van", title: "Công văn trao đổi", desc: "Quản lý công văn", icon: <Send size={24} /> },
            { key: "tu-hinh", title: "Hồ sơ tử hình", desc: "Quản lý hồ sơ tử hình", icon: <FileText size={24} /> },
          ].map(item => {
            ...
            return (
              <div
                ...
                style={{
                  ...
                  transition: 'all 0.2s',
                  ...(isSelected ? { borderColor: RED } : {})
                }}
              >
                <div style={{ marginRight: 16 }}>
                  {React.cloneElement(item.icon as any, { color: isSelected ? RED : "#6b7280" })}
                </div>
                <div>
                  <div style={{ fontWeight: 600, fontSize: 15, color: isSelected ? RED : '#374151' }}>{item.title}</div>
                </div>
              </div>
            );
          })}
        ...
        {tab === 'tu-hinh' && (
          <div>
            <Tabs
              activeKey={tuHinhSubTab}
              onChange={(k) => setTuHinhSubTab(k as "den" | "di")}
              tabBarExtraContent={
                <Space>
              ...
              items={[
                { key: 'den', label: 'Hồ sơ tử hình đến' },
                { key: 'di', label: 'Hồ sơ tử hình đi' }
              ]}
            />
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <Radio.Group value={filterStatus} onChange={e => setFilterStatus(e.target.value)}>
                <Radio value="tat-ca"><strong>Tất cả</strong></Radio>
                <Radio value="chua-xu-ly">{tuHinhSubTab === 'di' ? 'Chưa chuyển' : 'Chưa tiếp nhận'} <span style={{ fontSize: '90%' }}>({tuHinhSubTab === 'den' ? thuHinhChua : 0})</span></Radio>
                <Radio value="da-xu-ly">{tuHinhSubTab === 'di' ? 'Đã chuyển' : 'Đã tiếp nhận'}</Radio>
                <Radio value="da-tra">{tuHinhSubTab === 'di' ? 'Bị trả lại' : 'Trả lại'}</Radio>
              </Radio.Group>
              <Input prefix={<Search size={16} color="#9ca3af" />} placeholder="Tìm kiếm theo số hiệu, đương sự..." style={{ width: 300 }} onChange={e => setSearchText(e.target.value)} allowClear />
            </div>
            <Table
              rowSelection={{ type: "checkbox" }}
              columns={tuHinhCols}
              dataSource={getFilteredData(tuHinhSubTab === 'den' ? listTuHinhDen : [])}
              rowKey="id"
        ...
        {tab === 'cong-van' && (
          <div>
            <Tabs
              activeKey={congVanSubTab}
              onChange={(k) => setCongVanSubTab(k as "den" | "di")}
              tabBarExtraContent={
                <Space>
              ...
              items={[
                { key: 'den', label: 'Công văn đến' },
                { key: 'di', label: 'Công văn đi' }
              ]}
            />
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <Radio.Group value={filterStatus} onChange={e => setFilterStatus(e.target.value)}>
                <Radio value="tat-ca"><strong>Tất cả</strong></Radio>
                <Radio value="chua-xu-ly">{congVanSubTab === 'di' ? 'Chưa chuyển' : 'Chưa tiếp nhận'} <span style={{ fontSize: '90%' }}>({congVanSubTab === 'den' ? cvDenChua : cvDiCho})</span></Radio>
                <Radio value="da-xu-ly">{congVanSubTab === 'di' ? 'Đã chuyển' : 'Đã tiếp nhận'}</Radio>
                <Radio value="da-tra">{congVanSubTab === 'di' ? 'Bị trả lại' : 'Trả lại'}</Radio>
              </Radio.Group>
              <Input prefix={<Search size={16} color="#9ca3af" />} placeholder="Tìm kiếm theo số hiệu, đương sự..." style={{ width: 300 }} onChange={e => setSearchText(e.target.value)} allowClear />
            </div>
            <Table
              rowSelection={{ type: "checkbox" }}
              columns={congVanSubTab === 'den' ? cvDenCols : cvDiCols}
              dataSource={getFilteredData(congVanSubTab === 'den' ? listCongVanDen : listCongVanDi)}
              rowKey="id"
              pagination={{ pageSize: 10 }}
              size="middle"
              scroll={{ y: "calc(100vh - 400px)" }}
        ...
        {tab === 'khang-nghi' && (
          <div>
            <Tabs
              activeKey={kNSubTab}
              onChange={(k) => setKNSubTab(k as "den" | "di")}
              tabBarExtraContent={
                <Space>
                  {kNSubTab === "den" && <Button type="primary" onClick={() => onThemHoSo ? onThemHoSo() : setShowNhapTay("khang-nghi")}> Thêm mới HSKN</Button>}
                  {kNSubTab === "den" && <Button style={{ color: "#0f7b52", borderColor: "#0f7b52" }} onClick={() => { const f = listKNDen.find(x => x.trangThai === "Chưa tiếp nhận"); if (f) handleNhanKNDen(f.id); }}>Nhận hồ sơ</Button>}
              ...
              items={[
                { key: 'den', label: 'Hồ sơ kháng nghị đến' },
                { key: 'di', label: 'Hồ sơ kháng nghị đi' }
              ]}
            />
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <Radio.Group value={filterStatus} onChange={e => setFilterStatus(e.target.value)}>
                <Radio value="tat-ca"><strong>Tất cả</strong></Radio>
                <Radio value="chua-xu-ly">{kNSubTab === 'di' ? 'Chưa chuyển' : 'Chưa tiếp nhận'} <span style={{ fontSize: '90%' }}>({kNSubTab === 'den' ? knDenCho : knDiCho})</span></Radio>
                <Radio value="da-xu-ly">{kNSubTab === 'di' ? 'Đã chuyển' : 'Đã tiếp nhận'}</Radio>
                <Radio value="da-tra">{kNSubTab === 'di' ? 'Bị trả lại' : 'Trả lại'}</Radio>
              </Radio.Group>
              <Input prefix={<Search size={16} color="#9ca3af" />} placeholder="Tìm kiếm theo số hiệu, đương sự..." style={{ width: 300 }} onChange={e => setSearchText(e.target.value)} allowClear />
            </div>
            <Table
              ...
              dataSource={getFilteredData(kNSubTab === "den" ? listKNDen : listKNDi)}
              rowKey="id"
              pagination={{ pageSize: 10 }}
              size="middle"
              scroll={{ y: "calc(100vh - 400px)" }}
        ...
        {tab === 'xet-xu-lai' && (
          <div>
            <Tabs
              ...
              items={[
                { key: 'di', label: 'Hồ sơ xét xử lại đi' }
              ]}
            />
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <Radio.Group value={filterStatus} onChange={e => setFilterStatus(e.target.value)}>
                <Radio value="tat-ca"><strong>Tất cả</strong></Radio>
                <Radio value="chua-xu-ly">Chưa chuyển <span style={{ fontSize: '90%' }}>({xxlChoXuLy})</span></Radio>
                <Radio value="da-xu-ly">Đã chuyển</Radio>
                <Radio value="da-tra">Bị trả lại</Radio>
              </Radio.Group>
              <Input prefix={<Search size={16} color="#9ca3af" />} placeholder="Tìm kiếm theo số hiệu, đương sự..." style={{ width: 300 }} onChange={e => setSearchText(e.target.value)} allowClear />
            </div>
            <Table
              rowSelection={{ type: "checkbox" }}
              columns={xxlCols}
              dataSource={getFilteredData(listXetXuLai)}
              rowKey="id"
              pagination={{ pageSize: 10 }}
              size="middle"
              scroll={{ y: "calc(100vh - 400px)" }}
        ...
        {showNhapTay && <ModalNhapTayHoSoDen loaiHoSo={showNhapTay} onClose={() => setShowNhapTay(null)} onSave={data => {
          if (showNhapTay === "tu-hinh") {
            ...
            setListKNDen(prev => [...prev, { id: data.id, soHieuKN: data.soCongVan, ngayKN: data.ngayGui || "—", nguoiKN: data.nguoiGui, soBA: "—", toaRaBanAn: "—", donViGui: data.tenDonVi, ngayNhan: "--", nguoiNhan: "--", trangThai: "Chưa tiếp nhận" }]);
          }
          setShowNhapTay(null);
...
export default function SafeQuanLyHoSoGiaoNhanView(props: any) {
  return <ErrorBoundary><QuanLyHoSoGiaoNhanView {...props} /></ErrorBoundary>;
}
